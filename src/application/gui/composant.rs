...
impl EditText {
    pub fn show(&mut self, ui: &mut Ui,   edit_file: &mut EditFile) -> Result<()> {
        Window::new(&edit_file.filename)
            ...
            |ui|
            {
                ui.add_sized([ui.available_height() -50.0, ui.available_width() - 50.0], TextEdit::multiline(&mut edit_file.contenu));
            
                if ui.button("Enregistrer").clicked() {
                    let write = File::options().read(true).write(true).open(construct_path(&edit_file.filename))
                        .and_then(|mut f| 
    ...
    pub fn open(&mut self ,filename: String, edit_reflexion:&mut EditFile) {
        info!("Opening: {}", construct_path(&filename));
        edit_reflexion.contenu = read_to_string( construct_path(&filename)).unwrap();
