...
impl TryFrom<&str> for Attribut {
    ...
    fn try_from(value: &str) -> Result<Self, Self::Error> {
        let splitted = value.trim().split_once("=")
            .ok_or(ApplicationError::DefaultError("Could not parse Attribute: ".to_string() + value))?;
        Ok(Self{key: splitted.0.to_string(), value:splitted.1.to_string()})
    }
}

pub fn new_from_array(array: &String) -> Result<Vec<Attribut>, ApplicationError> {
    array.split(",").map(Attribut::try_from).collect::<Result<Vec<Attribut>,ApplicationError>>()
}
